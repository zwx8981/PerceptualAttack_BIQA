...
def brisque(x: torch.Tensor,
    ...
    for _ in range(num_of_scales):
        ...
        x = F.interpolate(x, size=(x.size(2) // 2, x.size(3) // 2), mode=interpolation, align_corners=True)
...
def _scale_features(features: torch.Tensor) -> torch.Tensor:
    ...
    feature_ranges = scio.loadmat('./scale.mat')
    feature_ranges = feature_ranges['scale']
    feature_ranges = torch.from_numpy(feature_ranges)
    feature_ranges = feature_ranges.t().to(features)

    # feature_ranges = torch.tensor([[0.338, 10], [0.017204, 0.806612], [0.236, 1.642],
    #                                [-0.123884, 0.20293], [0.000155, 0.712298], [0.001122, 0.470257],
    #                                [0.244, 1.641], [-0.123586, 0.179083], [0.000152, 0.710456],
    #                                [0.0009267, 0.470984], [0.249, 1.555], [-0.135687, 0.100858],
    #                                [0.000175, 0.684173], [0.000868, 0.534174], [0.258, 1.561],
    #                                [-0.143408, 0.100486], [0.000179, 0.685696], [0.000843, 0.536508],
    #                                [0.424, 3.264], [0.0205, 0.703171], [0.247, 1.046],
    #                                [-0.1560, 0.187459], [0.00013087, 0.442057], [0.0016, 0.40803],
    #                                [0.243, 1.042], [-0.1736, 0.162604], [1.2311e-4, 0.444362],
    #                                [0.0017, 0.40243], [0.254, 0.996],
    #                                [-0.1557, 0.09832299999999999], [1.4392e-4, 0.531903],
    #                                [0.001122, 0.369589], [0.228, 0.99], [-0.135370865464211, 0.098658],
    #                                [1.44599e-04, 0.530092], [0.0014, 0.370399]]).to(features)
...
def _score_svr(features: torch.Tensor) -> torch.Tensor:
    ...
    # Source: https://live.ece.utexas.edu/research/Quality/index_algorithms.htm
    gamma = 0.05
    rho = 30.168717009069866
    model = scio.loadmat('./svm_model.mat')
    sv_coef = model['sv_coef']
    sv = model['SVs']
    sv_coef = torch.from_numpy(sv_coef).to(features.device)
    sv_coef = sv_coef.squeeze()
    sv = torch.from_numpy(sv).to(features.device)
    #rho = -153.591
    sv.t_()
