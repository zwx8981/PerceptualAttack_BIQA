...
def ssim(x: torch.Tensor, y: torch.Tensor, kernel_size: int = 11, kernel_sigma: float = 1.5,
         data_range: Union[int, float] = 1., reduction: str = 'mean', full: bool = False, original_map = False,
    ...
    """
    _validate_input(
        input_tensors=(x, y), allow_5d=True, kernel_size=kernel_size, scale_weights=None, data_range=data_range, allow_negative=True)
    ...
    ssim_map, cs_map, ori_ssim_map, ori_cs_map = _compute_ssim_per_channel(x=x, y=y, kernel=kernel, data_range=data_range, k1=k1, k2=k2)
    ...
    if original_map:
        return ssim_val, ori_ssim_map, ori_cs_map
...
def _ssim_per_channel(x: torch.Tensor, y: torch.Tensor, kernel: torch.Tensor,
    ...
    mu1 = F.conv2d(F.pad(x, mode='replicate', pad=(5,5,5,5)), weight=kernel, stride=1, padding=0, groups=n_channels)
    mu2 = F.conv2d(F.pad(y, mode='replicate', pad=(5,5,5,5)), weight=kernel, stride=1, padding=0, groups=n_channels)
    ...
    compensation = 1.0
    sigma1_sq = compensation * (F.conv2d(F.pad(x * x, mode='replicate', pad=(5,5,5,5)), weight=kernel, stride=1, padding=0, groups=n_channels) - mu1_sq)
    sigma2_sq = compensation * (F.conv2d(F.pad(y * y, mode='replicate', pad=(5,5,5,5)), weight=kernel, stride=1, padding=0, groups=n_channels) - mu2_sq)
    sigma12 = compensation * (F.conv2d(F.pad(x * y, mode='replicate', pad=(5,5,5,5)), weight=kernel, stride=1, padding=0, groups=n_channels) - mu1_mu2)
    ...
    #ssim_val = torch.min(torch.min(ssim_map, dim=-1)[0],dim=-1)[0]

    cs = cs_map.mean(dim=(-1, -2))

    # ssim_val = ssim_map.min().unsqueeze(0).unsqueeze(0)
    # cs = cs_map.min().unsqueeze(0).unsqueeze(0)
...
def _multi_scale_ssim(x: torch.Tensor, y: torch.Tensor, data_range: Union[int, float], kernel: torch.Tensor,
    ...
    for iteration in range(levels):
        ...
        ssim_val, cs, _, _ = _ssim_per_channel(x, y, kernel=kernel, data_range=data_range, k1=k1, k2=k2)
        mcs.append(cs)
